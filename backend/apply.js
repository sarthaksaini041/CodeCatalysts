...
// Initialize Supabase only when requested
const getSupabase = () => {
...
export default async function handler(req, res) {
  // Only allow POST requests for the apply form
  if (req.method !== 'POST') {
  ...
  if (!supabase) {
    return res.status(500).json({ error: 'Database configuration missing from server.' });
  }
  ...
  try {
    ...
    // Format payload to match the database schema
    const payload = {
      name: data.name.trim(),
      ...
      portfolio: '', // Optional in old schema
      domain: data.domain,
