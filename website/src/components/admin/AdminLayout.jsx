...
export default function AdminLayout() {
  ...
  const pageTitle = useMemo(() => {
    const matchedItem = [...navItems].reverse().find((item) => location.pathname === item.to);
    return matchedItem?.label || 'Admin Portal';
  }, [location.pathname]);
      ...
      <div className="admin-layout">
        <aside className="admin-sidebar">
          <div className="admin-brand">
            <Link to="/" className="admin-brand-mark" aria-label="Code Catalysts home">
              <svg viewBox="108 254 722 430" fill="currentColor" xmlns="http://www.w3.org/2000/svg" style={{ width: '22px', height: '18px' }}>
                <path d={CC_LOGO_PATH} />
              </svg>
            </Link>
            <div className="admin-brand-title">Code Catalysts Admin</div>
            <p className="admin-brand-copy">
              Manage the live website content without touching `data.js`.
            </p>
          </div>
        ...
        <main className="admin-main">
          <div className="admin-topbar">
            ...
            <div className="admin-topbar-actions">
              {user?.email && <div className="admin-user-chip">{user.email}</div>}
